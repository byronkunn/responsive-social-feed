...
import { createList } from "@/lib/social-api";
...
function Lists() {
  const [allLists, setAllLists] = useState<PulseList[]>(initialLists);
  const [followed, setFollowed] = useState<string[]>(["quiet-web"]);
    ...
    <AppShell>
      ...
      <ul className="divide-y divide-border">
        {allLists.map((list) => {
          ...
          return (
            <li key={list.slug} className="px-4 py-4 transition-colors hover:bg-surface/60 sm:px-6">
